# ================================================================
#   FRIEDEL-CRAFTS: ELEY-RIDEAL (A adsorbed) MODEL FITTING
#   (Per-Temperature Correction + SSE and R² Added)
# ================================================================
...
def fitting_rate_law_er_A(t, XA, T, M, CA0, k_s, K_A, K_B):
    """
    Defines the ODE for the Eley-Rideal model with A adsorbed.
...
for temp in unique_temps:
    ...
    if result.success:
        ...
        # --- Compute SSE and R² for this temperature ---
        sse_temp, ss_tot_temp = 0, 0
        for run_data in experimental_runs.values():
            if run_data['T'] != temp:
                continue
            time_exp = np.insert(np.array(run_data['time']), 0, 0)
            ...
            conv_model = sol.y[0]
            residuals = conv_exp - conv_model
            sse_temp += np.sum(residuals**2)
            ss_tot_temp += np.sum((conv_exp - np.mean(conv_exp))**2)

        r2_temp = 1 - (sse_temp / ss_tot_temp) if ss_tot_temp > 0 else 0

        # Store per-temperature results including SSE and R²
        fitted_params_map[temp] = {
            'ks': ks_fit, 'KA': KA_fit, 'KB': KB_fit,
            'SSE': sse_temp, 'R2': r2_temp
        }
        fitted_ks_map[temp] = ks_fit # For Arrhenius plot
        print(f"  k_s: {ks_fit:.5f} | K_A: {KA_fit:.4f} | K_B: {KB_fit:.4f} | SSE: {sse_temp:.5f} | R²: {r2_temp:.4f}")
    else:
        print(f"  ERROR: Fit failed for {temp} K.")
        fitted_params_map[temp] = {'ks': np.nan, 'KA': np.nan, 'KB': np.nan, 'SSE': np.nan, 'R2': np.nan}
...
#
print("\n\n=======================================================")
print("--- PART 3: Final Eley-Rideal (A ads) Results Summary ---")
print("=======================================================")
...
print("\n--- Per-Temperature Fitted Adsorption Constants and Fit Quality (±5%) ---")
for temp, params in fitted_params_map.items():
    ka_val = params['KA']
    kb_val = params['KB']
    sse_val = params.get('SSE', np.nan)
    r2_val = params.get('R2', np.nan)
    print(f"Temp: {temp} K")
    print(f"  K_A (Adsorption Const): {ka_val:.4f} ± {ka_val*error_margin:.4f}")
    print(f"  K_B (Constant):         {kb_val:.4f} ± {kb_val*error_margin:.4f}")
    print(f"  SSE (Fit Error):        {sse_val:.5f}")
    print(f"  R²  (Fit Quality):      {r2_val:.4f}\n")
