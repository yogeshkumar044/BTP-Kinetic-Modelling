# ================================================================
#   FRIEDEL-CRAFTS: LHHW SINGLE-SITE MODEL FITTING
...
file_path = 'FCR_BTP2.csv'  # <-- Correct file for Friedel-Crafts
...
# --- Initial Conditions for Friedel-Crafts ---
...
#   Parameters: [A_s, Ea, KA_343, KB_343, KA_353, KB_353, KA_363, KB_363]
#
print("\n\n=====================================================================")
...
def fitting_rate_law_lhhw_ss(t, XA, T, M, CA0, A_s, Ea, K_A, K_B):
    """
    Defines the ODE for the LHHW single-site model with both reactants adsorbed.
...
def sum_of_squared_errors_lhhw_ss(params):
    ...
    params = [A_s, Ea, KA_343, KB_343, KA_353, KB_353, KA_363, KB_363]
    """
    total_error = 0
    A_s, Ea, KA_343, KB_343, KA_353, KB_353, KA_363, KB_363 = params
    ...
    for run_data in experimental_runs.values():
        ...
        if temp == 343:
            KA_run, KB_run = KA_343, KB_343
        elif temp == 353:
            KA_run, KB_run = KA_353, KB_353
        else: # 363 K
            KA_run, KB_run = KA_363, KB_363
...
A_s_fit, Ea_fit, KA_343_fit, KB_343_fit, KA_353_fit, KB_353_fit, KA_363_fit, KB_363_fit = result.x

# Store results in dictionaries for plotting and tables
fitted_params_map = {
    343: {'KA': KA_343_fit, 'KB': KB_343_fit},
    353: {'KA': KA_353_fit, 'KB': KB_353_fit},
    363: {'KA': KA_363_fit, 'KB': KB_363_fit}
}
...
print("\n--- Global Simultaneous Fit Results (LHHW Single-Site) ---")
...
for temp in temps_K:
    ...
    KA_run, KB_run = temp_params['KA'], temp_params['KB']
    ...
    for run_data in experimental_runs.values():
        ...
        sol = solve_ivp(
            fun=lambda t, y: fitting_rate_law_lhhw_ss(
                t, y, run_data['T'], run_data['M'], run_data['CA0'],
                A_s_fit, Ea_fit, KA_run, KB_run # Use global fit params
            ),
            t_span=[0, max(time_exp) if len(time_exp) > 1 else 1],
            y0=[0],
            method='LSODA',
    ...
    # Print the summary
    print(f"  {temp} K: K_A = {KA_run:.4f}, K_B = {KB_run:.4f} | SSE: {sse_temp:.5f} | R²: {r_squared_temp:.4f}")
# --- END MODIFICATION ---
...
# --- PLOTTING MODIFICATION START ---
# Define colors for the molar ratios
colors = {
    1: 'blue',   # 1:1 ratio
    2: 'orange', # 1:2 ratio
    3: 'green'   # 1:3 ratio
...
for i, temp in enumerate(plots_data.keys()):
    ...
    for run_data in plots_data[temp]:
        ...
        plot_color = colors.get(molar_ratio_val, 'black') # Default to black if ratio not in map

        ax.scatter(time_exp, conv_exp, label=f'Molar Ratio 1:{molar_ratio_val} (Exp.)', s=50, color=plot_color)
        ...
        ax.plot(t_plot, sol_plot.y[0], label=f'Model Fit (M=1:{molar_ratio_val})', color=plot_color)
...
for i, temp in enumerate(plots_data.keys()):
    ...
    for run_data in plots_data[temp]:
        ...
        # Get the correct color
        plot_color = colors.get(molar_ratio_val, 'black')
        ...
        ax.plot(t_plot, sol_plot.y[0], linestyle='-', label=f'Fitted (M=1:{molar_ratio_val})', color=plot_color)
    ax.set_xlabel('Time (minutes)', fontsize=12); ax.grid(True); ax.legend()
...
# --- MODIFICATION: Added Overall SSE printout ---
