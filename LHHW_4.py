# ================================================================
#   FRIEDEL-CRAFTS: LHHW DUAL-SITE (All Species ads) MODEL FITTING
...
for temp in unique_temps:
    ...
    if result.success:
        ...
        # --- MODIFICATION: Calculate SSE and R² for this temp ---
        temp_exp_points = []
        temp_model_points = []
        for run_data in experimental_runs.values():
            if run_data['T'] != temp:
            ...
            sol = solve_ivp(
                fun=lambda t, y: fitting_rate_law_lhhw_ds_prod(
                    t, y, run_data['T'], run_data['M'], run_data['CA0'],
                    ks_fit, KA_fit, KB_fit, KC_fit, KD_fit # Use the just-fitted params
                ),
                t_span=[0, max(time_exp) if len(time_exp) > 1 else 1],
                y0=[0],
                method='LSODA',
        ...
        print(f"  k_s: {ks_fit:.5f} | K_A: {KA_fit:.4f} | K_B: {KB_fit:.4f} | K_C: {KC_fit:.4f} | K_D: {KD_fit:.4f} | SSE: {sse_temp:.5f} | R²: {r_squared_temp:.4f}")
